...
make package-app      # -> bin/app/InferMesh.app (macOS .app bundle, ad-hoc signed; byproduct raw binary stays at app/bin/InferMesh)
make package-app-dmg  # -> bin/app/InferMesh.dmg (macOS; darwin-only, builds package first)
make package-app-linux    # -> bin/app/ (AppImage + deb; Linux host/wails-cross Docker only — guidance + exit 1 elsewhere)
make package-app-windows  # -> compile-check (real cross-compile); currently FAILS until Windows desktop support lands (deferred); + NSIS installer when makensis present
make test-app-smoke   # build, launch, liveness-only smoke test (needs a display; see Testing)
# These make targets prepend `$(go env GOPATH)/bin` to PATH, so a `go install`-ed
# `wails3` resolves even when it is not on your shell PATH.
# package-app-windows is a compile-check ONLY — it does NOT produce an exe today.
# The app's process supervisor uses Unix-only syscalls (syscall.Setpgid /
# syscall.Kill in app/supervisor_process.go) that do not exist on GOOS=windows;
# Windows desktop support is deferred (see Design constraints). The target is
# wired correctly (wails3 windows Taskfile + NSIS/MSIX config with the InferMesh
# identity), so bin/app/InferMesh.exe + the NSIS installer will be produced
# automatically once Windows support lands.
...
make test-app-smoke   # NOT part of `make test`: builds + launches the desktop app, liveness-only smoke test. Requires a display — skips headless Linux (exit 0); macOS-runnable in CI (SKIP=1 / INFERMESH_SMOKE_SKIP=1 / INFERMESH_APP_BIN env overrides).
...
app/                          # Wails v3 desktop app (separate Go module, replace => ../)
  ...
  smoke_test.go               # build tag `smoke` (excluded from normal `go test`); launches the built app, asserts liveness
...
# app packaging artifacts land in bin/app/ (root-level, gitignored): infermesh-app
# (raw build), InferMesh.app/.dmg (macOS), InferMesh.exe (Windows — once Windows
# support lands; package-app-windows currently fails, see Build & run),
# InferMesh-*.AppImage + *.deb (Linux). See `make package-app*` targets.
