...
tmux split-window -v -t "$SESSION_NAME:0" "./bin/infermesh-worker --dev-mode --model-path /path/to/model.gguf --backend llama-cpp"

# 3. Split the bottom pane horizontally to create Pane 2 (Bottom-Right)
tmux split-window -h -t "$SESSION_NAME:0.1" "curl -X POST http://localhost:8080/v1/chat/completions -H 'Content-Type: application/json' -d '{\"model\": \"gpt-oss-20b\", \"messages\": [{\"role\": \"user\", \"content\": \"Hello\"}], \"stream\": false}'; exec $SHELL"
